...
function rotate(array, steps){
  ...
  if ( steps > 0 ){
    var arr = array.slice(-steps);
    return  arr.concat(array.splice(0,array.length-steps));	
  }else if ( steps === 0 ) {
  	return array;
  }else {
    var arr = array.slice(0,-steps);
	return array.slice(-steps,array.length).concat(arr)
  }
} 
...
function nextBigger(num){
  return Number(num.toString(10).match(/[0-9]/gi).sort(function (a,b) {
    return b-a;
   }).join(''));
}
